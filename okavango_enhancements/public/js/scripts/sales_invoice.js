...
erpnext.accounts.SalesInvoiceController = class SalesInvoiceController extends (
	...
	// add set_batch_number and _set_batch_number
	...
	_set_batch_number(doc) {
		...
		let args = {'item_code': doc.item_code, 'qty': flt(doc.qty) * flt(doc.conversion_factor)};
		if (doc.has_serial_no && doc.serial_no) {
			args['serial_no'] = doc.serial_no
		}

		return frappe.call({
			method: 'okavango_enhancements.app.get_single_batch_no',
			//erpnext.stock.doctype.batch.batch.get_batch_no',
			args: args,
			callback: function(r) {
				if(r.message) {
					frappe.model.set_value(doc.doctype, doc.name, 'warehouse', r.message[1]);
					frappe.model.set_value(doc.doctype, doc.name, 'batch_no', r.message[0]);
				} else {
					frappe.model.set_value(doc.doctype, doc.name, 'warehouse', r.message[1]);
				    frappe.model.set_value(doc.doctype, doc.name, 'batch_no', r.message[0]);
				}
